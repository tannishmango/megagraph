...
import { PutCollateral, TakeCollateral, Borrow, Repay, Liquidate, AlphaHomoraV2HomoraBank} from "../../../generated/AlphaHomoraV2HomoraBank/AlphaHomoraV2HomoraBank";
...
import { AlphaHomoraV2WMasterChef, AlphaHomoraV2FarmData, AlphaHomoraV2LpPair, AlphaHomoraV2LendData} from "../../../generated/schema";
import { getOrCreateCyToken, getOrCreateToken } from './PoolRegistry'
import { convertBINumToDesiredDecimals } from '../../utils/converters'
import { homoraBankAddress,ZERO_ADDRESS } from '../../utils/constants'



function saveJoePoolInfo(farmData:AlphaHomoraV2FarmData, pid: i32, wChefId:string, chefAddress:string, block: BigInt, timestamp: BigInt): void {
...
function saveJoePoolV3Info(farmData:AlphaHomoraV2FarmData, pid: i32, wChefId:string, chefAddress:string, block: BigInt, timestamp: BigInt): void {
  let chef = AlphaHomoraV2MasterChef.bind(Address.fromString(chefAddress))
  let poolInfoCall = chef.try_poolInfo(BigInt.fromI32(pid))
  let totalAllocCall = chef.try_totalAllocPoint()
  let rewardPerSecondCall = chef.try_joePerSec()
  ...
  if (!poolInfoCall.reverted && !totalAllocCall.reverted && !rewardPerSecondCall.reverted && !userInfoCall.reverted && !pendingTokensCall.reverted){
    let poolInfo = poolInfoCall.value
    let lpToken = AlphaHomoraV2LpPair.load(poolInfo.value0.toHexString())
    if (lpToken){
      farmData.pid = pid
...
function savePngPoolInfo(farmData:AlphaHomoraV2FarmData, pid: i32, wChefId:string, chefAddress:string, block: BigInt, timestamp: BigInt): void{
  ...
  if (!poolInfoCall.reverted && !totalAllocCall.reverted && !rewardPerSecondCall.reverted && !userInfoCall.reverted && !lpTokenCall.reverted && !rewarderCall.reverted){
    let poolInfo = poolInfoCall.value
...
export function handlePutCollateral(event: PutCollateral): void {
  ...
  let pid = BigInt.fromString(event.params.id.toHexString().slice(0,4)).toI32()
  if (pid){
    if (wChef.id == "0x1f806f7c8ded893fd3cae279191ad7aa3798e928"){
      savePngPoolInfo(farmData,pid,event.params.token.toHexString(),wChef.chef,event.block.number,event.block.timestamp)
    }
    if (wChef.id == "0x188bed1968b795d5c9022f6a0bb5931ac4c18f00"){
      saveJoePoolV3Info(farmData,pid,event.params.token.toHexString(),wChef.chef,event.block.number,event.block.timestamp)
    }
    else{
      saveJoePoolInfo(farmData,pid,event.params.token.toHexString(),wChef.chef,event.block.number,event.block.timestamp)
    }
  }
...
export function handleBorrow(event:Borrow): void{
  let homoraBankContract = AlphaHomoraV2HomoraBank.bind(homoraBankAddress)
  ...
  if (!crTokenAddressCall.reverted){
    ...
    let accountSnapshotCall = crTokenContract.try_getAccountSnapshot(homoraBankAddress)
    if (!borrowRatePerBlock.reverted && !supplyRatePerBlock.reverted && !accountSnapshotCall.reverted){
      ...
      alphaLendData.cyTokenExchangeRate = convertBINumToDesiredDecimals(accountSnapshotCall.value.value3,10+underlyingDecimals)
      alphaLendData.save()
    }
  }
}
